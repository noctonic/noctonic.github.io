...
function updateSwatch(r, g, b) {
  ...
  if (currentColorEl) {
    // Try to find an existing child swatch element
    let swatch = currentColorEl.querySelector(".swatch");
    if (!swatch) {
      // If not found, clear the container and create a new swatch element
      currentColorEl.innerHTML = "";
      swatch = document.createElement("div");
      swatch.className = "swatch";
      // Set fixed dimensions and any additional styling you need
      swatch.style.width = "30px";
      swatch.style.height = "30px";
      swatch.style.borderRadius = "2px";
      currentColorEl.appendChild(swatch);
    }
    // Update only the child swatch's background color
...
function updateSchemesDistanceShadowSpotlight(h, s, bri) {
  ...
  if (currentColorContainer) {
    ...
    // Add a single swatch with the current color
