...
fn initBaseBlock() {
    ...
    for (var i = 0u; i < pLen; i = i + 1u) {
        let cVal = prefixChars[i] & 0xFFu;
        let bytePos = i * 2u;

        let wIndex = bytePos >> 2u;
        let shift  = (bytePos & 3u) * 8u;
        sharedBaseBlock[wIndex] |= (cVal << shift);

    }

    let totalBytes   = tLen * 2u;
    let padWordIndex = totalBytes >> 2u;
    let padShift     = (totalBytes & 3u) * 8u;
    sharedBaseBlock[padWordIndex] |= (0x80u << padShift);

    sharedBaseBlock[14] = totalBytes << 3u;
}
...
fn buildBlockWithSuffix(suffixId : u32) -> array<u32, 16> {
    ...
    for (var i = 0u; i < sLen; i = i + 1u) {
        ...
        let cVal = sharedCharset[digit] & 0xFFu;

        let charPos = pLen + i;
        let bytePos = charPos * 2u;

        let wIndex = bytePos >> 2u;
        let shift  = (bytePos & 3u) * 8u;
        block[wIndex] |= (cVal << shift);
