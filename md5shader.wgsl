...
var<workgroup> sharedPrefixBlock   : array<u32, 16>;
var<workgroup> sharedPrefixLength  : u32;
...
fn buildSharedPrefixBlock() {
    for (var i = 0u; i < 16u; i = i + 1u) {
        sharedPrefixBlock[i] = 0u;
    }

    let pLen = config.prefixLength;
    sharedPrefixLength = pLen;

    for (var i = 0u; i < pLen; i = i + 1u) {
        let bVal   = prefixChars[i] & 0xFFu;
        let wIndex = i >> 2u;
        let shift  = (i & 3u) * 8u;
        sharedPrefixBlock[wIndex] = sharedPrefixBlock[wIndex] | (bVal << shift);
    }
}
...
fn buildBlockWithSuffix(suffixId : u32) -> array<u32,16> {
    ...
    for (var i = 0u; i < 16u; i = i + 1u) {
        block[i] = sharedPrefixBlock[i];
    }

    let pLen      = sharedPrefixLength;
    ...
    let dataBytesUsed = pLen + sLen;
    if (dataBytesUsed < 64u) {
        let padWordIndex = dataBytesUsed >> 2u;
        let shift        = (dataBytesUsed & 3u) * 8u;
